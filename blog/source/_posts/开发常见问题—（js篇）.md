...
有时会出现多个组件 watch 同一个 VueX 中 state 的值，这时会同时触发多个组件中对应的 watch 函数，可以通过判断当前路由来解决，而对于使用组件缓存视图和非缓存视图而言，可以在 keep-alive 中的生命周期中增加一个判断标识来解决，如果是新视图，关闭缓存视图中的 watch 函数相关事件