...
## 合并部分文件
比如 dev 分支想要合并 another 分支上的 /src/assets 文件夹
切换到 dev 分支，并执行以下命令
```
git checkout another /src/assets
```

## .gitignore 修改后重新追踪修改的文件（夹）
.gitignore 只能忽略“未被 Git 跟踪”的文件，若文件已被 Git 跟踪，修改 .gitignore 后需用 git rm --cached 手动从缓存中移除，才能真正让 .gitignore 生效。

```bash
git rm --cached .env # 移除指定文件的索引
git rm -r --cached . # 递归移除全部索引
git add .
git commit -m "chore: remove .env now ignored by .gitignore"
```

git rm -r --cached . 会从 Git 索引中移除项目中所有当前已跟踪的文件，但不会删除本地文件内容，这是让 .gitignore 生效的标准操作，后续记得重新 add . 和 commit 一次。
