...
但是五张图片全部72度，两侧的两张图片观感不是很好，所以进行微调一下，两侧的两张图片分别偏移12度，后续再动态修改回72度的倍数就好了。
  ...
  methods: {
    ...
    rotateCarousel(direction, round = 1, reset = false) {
        ...
        this.carouselList.forEach((item, index) => {
            if (this.currentIndex + 1 == index || (this.currentIndex == 4 && index == 0)) {
                ...
                Math.abs(this.currentIndex - index) == 2 ||
                Math.abs(this.currentIndex - index) == 3
            ) {
                // 翻转后方两张图片
                item.imgStyle = 'transform: rotateY(180deg)'
                item.style = `transform: rotateY(${item.rotate}deg) translateZ(7rem) scale(0.9)`
            } else {
                item.style = ''
                item.imgStyle = ''
            }
    ...
    handleCenterInfo(direction, index) {
        ...
        if (direction > 0) {
            ...
            this.currentCarousel =
                carouselCenterList[
                    this.carouselList[nextItemIndex].imgIndex == 0
                        ? 4
                        : this.carouselList[nextItemIndex].imgIndex - 1
                ]
        }
    },
