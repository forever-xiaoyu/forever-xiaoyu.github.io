...
### 启动页
IOS
用同一storyboard的情况下，会出现启动图出现多张的情况，产生了缓存，目前解决方法是每次更换启动图生成一份新的storyboard，可能zip名称重复也会有这个问题。
比较坑的是只能打正式包或自定义基座才能看到效果，因为云打包每天有免费次数限制（这个官方说法是会动态调整，尝试过后大约是8-10次，安卓和ios分别消耗次数），这两种方式都消耗次数，所以要注意次数消耗，当然也可以付费购买打包次数。
